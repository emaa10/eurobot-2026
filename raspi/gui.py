        ...
        while True:
            ...
            try:
                ...
                if cmd == 'p':
                    ...
                    print("Debug: pullcord pulled.")
                elif cmd == 'h':
                    if not self.homing_1_done:
                        self.homing_1_done = True
                        print("Debug: homing1 done.")
                    else:
                        self.homing_2_done = True
                        print("Debug: homing2 done.")
                elif cmd == 'c' and data[1:].isdigit():
                    self.points = int(data[1:])
                    print(f"Debug: points={self.points}")
                else:
                    print(f"Debug: Unknown cmd {data}")
            except Exception as e:
...
class MainWindow(QWidget):
    ...
    def _add_header(self, parent, back_index=None):
        ...
        btn_close.setStyleSheet('background:transparent; font-size:18px;')
    ...
    def init_start_screen(self):
        ...
        for name,col in [('Gelb','#FFD600'),('Blau','#2979FF')]:
            b = QPushButton(name)
            b.setStyleSheet(f'background:{col};color:#fff;padding:10px;border-radius:8px')
