...
class OptimizedMockCamera:
    ...
    def __init__(self, test_image_path: str, debug_cam: bool = False):
        ...
        self.output_dir = os.path.expanduser("~/Desktop/camera/")
    ...
    def check_perfect_stack(self, size: int) -> bool:
        """
        Perfekter Stack-Check basierend auf deinen Daten
        
        Returns:
            bool: True wenn Stack erfolgreich, False wenn nicht
        """
        ...
        # Visualisierung erstellen (nur bei Debug)
        if self.debug_cam:
            self.visualize_perfect_stack(size, results)
    ...
    def visualize_perfect_stack(self, size: int, results: dict):
        ...
        filename = os.path.join(self.output_dir, f'perfect_stack_{size}_{"success" if results["success"] else "failed"}.png')
...
def test_perfect_stack(debug_cam: bool = False):
    ...
    try:
        ...
        camera = OptimizedMockCamera(test_image_path, debug_cam=debug_cam)
        ...
        for stack_size in [1, 2, 3]:
            ...
            success = camera.check_perfect_stack(stack_size)
            results[stack_size] = success
            ...
            if debug_cam:
                ...
                # Kurze Ausgabe ohne Debug
                print(f"Stack {stack_size}: {success}")

        if debug_cam:
            print(f"\n🏁 === TEST ABGESCHLOSSEN ===")
            print("📁 Alle Visualisierungen wurden im ~/Desktop/camera/ Ordner gespeichert.")
...
if __name__ == "__main__":
    ...
    test_perfect_stack(debug_cam=True)
    
    # Test ohne Debug-Ausgaben (nur True/False)
    # test_perfect_stack(debug_cam=False)
